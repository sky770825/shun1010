...
// 次月排班表（使用相同的排班條件，但寫入到「次月排班表」工作表）
function autoAssignNextMonth(){
  console.log('開始執行 autoAssignNextMonth（次月排班表）');
  const ym=document.getElementById('monthPicker').value;
  console.log('選擇的月份:', ym);
  const days=daysInMonth(ym);
  console.log('本月天數:', days);
  ...
  showConfirmModal(
    '📋 次月排班表計畫',
    plan,
    '確定要執行此分配計畫嗎？（寫入「次月排班表」）',
    () => {
      executeAutoAssignNextMonth();
    }
  );
}

function executeAutoAssignNextMonth(){
  console.log('開始執行 executeAutoAssignNextMonth（次月排班表）');
  const ym=document.getElementById('monthPicker').value;
  console.log('執行月份:', ym);
  ...
  // 使用與原本相同的排班邏輯（複製自 executeAutoAssign）
  let totalSlots=0;
  for(let d=1;d<=days;d++){
    const wd=new Date(`${ym}-${String(d).padStart(2,'0')}`).getDay();
    totalSlots += (wd===0||wd===6)? WEEKEND_SHIFTS.length : WEEKDAY_SHIFTS.length;
  ...
  // ⭐ 只發送到「次月排班表」，不影響本地數據和「排班記錄」
  (async () => {
    const success = await sendScheduleToNextMonthSheet(ym, data);
    if (success) {
      showSyncNotification('📊 次月排班表已同步到 Google Sheets');
    }
...
// 診斷成員排班狀態
function diagnoseMemberStatus(memberId) {
  const member = MEMBERS.find(m => m.id === memberId);
  
  if (!member) {
    return `❌ 找不到成員 ${memberId}`;
  }
  
  let report = `🔍 成員診斷報告：${memberId} ${member.name}\n\n`;
  
  // 檢查基本狀態
  report += `📋 基本狀態：\n`;
  report += `  - 是否禁用：${member.disabled ? '❌ 是（不能排班）' : '✅ 否'}\n`;
  report += `  - 是否在排除清單：${EXCLUDED_MEMBERS.includes(memberId) ? '❌ 是' : '✅ 否'}\n`;
  report += `  - 組隊標記：${member.group || '無'}\n\n`;
  
  // 檢查排班條件
  report += `📅 排班條件：\n`;
  
  // 特定日期限制
  const specificDays = SCHEDULE_CONDITIONS?.SPECIFIC_DAY_ONLY?.[memberId];
  if (specificDays) {
    report += `  - 只能排：${specificDays.map(d => getDayNameInChinese(d)).join('、')}\n`;
  } else {
    report += `  - 日期限制：無\n`;
  }
  
  // 禁止日期
  const forbiddenDays = SCHEDULE_CONDITIONS?.FORBIDDEN_DAYS?.[memberId];
  if (forbiddenDays) {
    report += `  - 不能排：${forbiddenDays.map(d => getDayNameInChinese(d)).join('、')}\n`;
  } else {
    report += `  - 禁止日期：無\n`;
  }
  
  // 禁止班別
  const forbiddenShifts = SCHEDULE_CONDITIONS?.FORBIDDEN_SHIFTS?.[memberId];
  if (forbiddenShifts) {
    report += `  - 禁止班別：${forbiddenShifts.join('、')}\n`;
  } else {
    report += `  - 班別限制：無\n`;
  }
  
  // 必須組合
  const partner = getRequiredPartner?.(memberId);
  if (partner) {
    const partnerMember = MEMBERS.find(m => m.id === partner);
    report += `  - 必須搭檔：${partner} ${partnerMember?.name || ''}\n`;
  }
  ...
  if (SCHEDULE_CONDITIONS?.FORBIDDEN_PAIRS) {
    Object.keys(SCHEDULE_CONDITIONS.FORBIDDEN_PAIRS).forEach(pair => {
      ...
      if (id1 === memberId) {
        const partnerMember = MEMBERS.find(m => m.id === id2);
        forbiddenPartners.push(`${id2} ${partnerMember?.name || ''}`);
      }
      if (id2 === memberId) {
        const partnerMember = MEMBERS.find(m => m.id === id1);
        forbiddenPartners.push(`${id1} ${partnerMember?.name || ''}`);
      }
    });
  }
  if (forbiddenPartners.length > 0) {
    report += `  - 不能搭檔：${forbiddenPartners.join('、')}\n`;
  }
  
  report += `\n結論：${member.disabled || EXCLUDED_MEMBERS.includes(memberId) ? '❌ 此成員無法參與排班' : '✅ 此成員可以正常參與排班'}\n`;
...
// 快速診斷 17 范沅
function diagnose17() {
  const report = diagnoseMemberStatus('17');
  showCustomAlert(report, 'info');
}
...
function showFullKeyItem(keyItem, itemCount, keyInfo) {
  ...
  // ⭐ 顯示分類和備註資訊框（優先顯示，在標題下方）
  if(keyInfo && (keyInfo.category || keyInfo.note)) {
    const infoBox = document.createElement('div');
    infoBox.style.cssText = 'margin:15px 0;padding:15px;background:linear-gradient(135deg, #667eea 0%, #764ba2 100%);border-radius:10px;box-shadow:0 4px 12px rgba(102,126,234,0.3);';
